...
# Complete Exclusive GPU Performance Testing Script
# Tests individual baselines + concurrent performance impact in one run
set -e

# Configuration
PHI35_ENDPOINT="http://localhost:8083"
DEEPSEEK_ENDPOINT="http://localhost:8084"
OUTPUT_DIR="test_results_exclusive"
...
echo -e "${BLUE}=== Complete Exclusive GPU Performance Analysis ===${NC}"
echo "Timestamp: $(date)"
...
test_concurrent_models() {
    echo -e "\n${BLUE}=== Testing Both Models Concurrently (Exclusive GPU Impact) ===${NC}"
...
if [[ "$phi35_active" == true ]] && [[ "$deepseek_active" == true ]]; then
    ...
    echo "To test GPU impact, ensure both models are running."
fi
...
echo "• Concurrent results show exclusive GPU impact"  
